// based on: https://github.com/pressly/goose/tree/main/cmd/goose/main.go
// - removed mysql options
// - removed validate command
// - added output option for log format selection
...
import (
	...
	"github.com/cryptogarageinc/goose-wrapper/v3/logger"
...
var (
	...
	// additional option:
	output = flags.String("output", "std", "output log format type: std/disable/slog/json")
)
...
func main() {
	...
	run(ctx, envConfig, args)
}

func run(ctx context.Context, envConfig *envConfig, args []string) {
...
func setOutput() {
	...
	case "slog":
		goose.SetLogger(logger.NewSlogDefaultLogger())
	case "json":
		goose.SetLogger(logger.NewSlogJsonLogger())
	default:
		// stdLogger is a default logger
	}
}
